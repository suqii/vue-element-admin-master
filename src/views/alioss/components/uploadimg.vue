      ...
      ><div class="grid-content left-con">
        <el-form ref="form">
          ...
          <el-form-item label="阿里云bucket">
            <el-input v-model="keySet.bucket" />
          </el-form-item>
          <!-- bucket选择 -->
          ...
          <el-form-item label="阿里云key">
            <el-input v-model="keySet.accessKeyId" />
          </el-form-item>
          <!-- secret -->
          <el-form-item label="阿里云secret">
            <el-input v-model="keySet.accessKeySecret" type="password" />
          </el-form-item>
          <!-- 区域选择 -->
...
export default {
  ...
  methods: {
    ...
    async filePathList(prefix, marker) {
      ...
      try {
        // 返回当前文件夹下的文件
        const rootPathGet = await client.list({
          prefix: '',
          delimiter: '/'
        })
        const rootPath = rootPathGet.prefixes
    ...
    async getPathLists(prefix) {
      try {
        const pathLists = await client.list({
          prefix: prefix,
          delimiter: '/'
        })
    ...
    handleRemove(file, fileList) {
      ...
      //   this.picList.push(this.showlist[i].url)
      // }
      // const result = client.delete(file.name)
      // console.log(result)
    },
    // 关闭预览图片
    closeViewer() {
